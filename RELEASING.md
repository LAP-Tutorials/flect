...
2. Add a finished section for that version to `CHANGELOG.md`. This becomes the public release description.
3. Commit and push the finished changes to `main`.
...
The command refuses to publish from a dirty tree, another branch, a commit that is not on `origin/main`, or a version without finished changelog notes. It runs the local checks, then starts the GitHub workflow. The workflow repeats the checks on standard Windows, verifies scrcpy and ADB, creates a versioned ZIP and SHA-256 checksum, and publishes `v<package version>` with the matching changelog section plus GitHub’s comparison link.
